' GRAMMAR ANALYZER
' By Lucian Wischik, (c) Microsoft 2009-2010
...
Module VBGrammarAnalyzer
    Sub Main()
        Dim fn = If(My.Application.CommandLineArgs.FirstOrDefault, "")
        Dim name = IO.Path.GetFileNameWithoutExtension(fn)
        Dim ext = IO.Path.GetExtension(fn).ToLower
        If String.IsNullOrEmpty(fn) OrElse (ext <> ".g" AndAlso ext <> ".ebnf") Then
            Console.WriteLine("Usage: VBGrammarAnalyzer [grammar.g | grammar.ebnf]")
            Console.WriteLine("This translates from the source grammar (either ANTLR or ISO14977 EBNF) to the other format, and also to HTML")
            Return
        End If

        Dim dstfn = IO.Path.ChangeExtension(fn, If(ext = ".g", ".ebnf", ".g"))
        Dim src = My.Computer.FileSystem.ReadAllText(fn)
        Dim productions = If(ext = ".g", Antlr.Parse(src), ISO14977.Parse(src))
        Dim time = My.Computer.FileSystem.GetFileInfo(fn).LastWriteTime
        If My.Computer.FileSystem.FileExists(dstfn) AndAlso My.Computer.FileSystem.GetFileInfo(dstfn).LastWriteTime > time Then
            Console.WriteLine("Error: target {0} is more recent than source {1}.", IO.Path.GetFileName(dstfn), IO.Path.GetFileName(fn))
            Console.WriteLine("It will not be overwritten. If you want to proceed, first delete the target.")
            Return
        End If

        Console.WriteLine(dstfn)
        Dim dst = If(ext = ".g", ISO14977.ToString(productions, name), Antlr.ToString(productions, name))
        My.Computer.FileSystem.WriteAllText(dstfn, dst, False, Text.Encoding.ASCII)
        IO.File.SetCreationTime(dstfn, time)
        IO.File.SetLastWriteTime(dstfn, time)


        Dim htmlfn = IO.Path.ChangeExtension(fn, ".html")
        Console.WriteLine(htmlfn)
        Dim dst_html = Html.ToString(productions, name)
        My.Computer.FileSystem.WriteAllText(htmlfn, dst_html, False, Text.Encoding.UTF8)
        IO.File.SetCreationTime(htmlfn, time)
        IO.File.SetLastWriteTime(htmlfn, time)
    End Sub
End Module


Class ISO14977
    Public Shared Shadows Function ToString(ByVal productions As LinkedList(Of Production), ByVal grammarName As String) As String
        Dim r = ""
    ...
    Public Shared Shadows Function ToString(ByVal ebnf As EBNF) As String
        Dim r = ""
    ...
    Public Shared Function Parse(ByVal src As String) As LinkedList(Of Production)
    ...
    Private Shared Function Tokenize(ByVal s As String) As LinkedList(Of String)
        s = s.Trim()
    ...
    Private Shared Sub GobbleUpComments(ByVal tokens As LinkedList(Of String), ByRef ExtraComments As String, ByRef HasNewline As Boolean)
    ...
    Private Shared Function ParseProduction(ByVal tokens As LinkedList(Of String), ByRef ExtraComments As String) As EBNF
    ...
    Private Shared Function ParsePar(ByVal tokens As LinkedList(Of String), ByRef ExtraComments As String) As EBNF
    ...
    Private Shared Function ParseSeq(ByVal tokens As LinkedList(Of String), ByRef ExtraComments As String) As EBNF
        Dim pp As New LinkedList(Of EBNF)
    ...
    Private Shared Function ParseAtom(ByVal tokens As LinkedList(Of String), ByRef ExtraComments As String) As EBNF
...
Class Html
    Public Shared Shadows Function ToString(ByVal productions As LinkedList(Of Production), ByVal grammarName As String) As String
        Dim html As New Html(productions, grammarName)
        html.Analyze()
    ...
    ' All other data-structures apart from CaseEscapedNames are keyed of strings like "production" or "'terminal'" or "'<extendedterminal>'"
    ...
    Shared Function normt(ByVal e As EBNF) As String
    ...
    End Function
    Shared Function normt(ByVal t As String) As String
        Return "'" & t & "'"
    End Function

    Sub New(ByVal productions As LinkedList(Of Production), ByVal grammarName As String)
        For Each p In productions
    ...
    Public Shadows Function ToString(ByVal grammarName As String) As String
...
{ var cc = bb[i].getElementsByTagName("a");
  ...
  { if (cc[j].name==ref || encodeURIComponent(cc[j].name)==ref) src=bb[i].parentNode.childNodes;
  }
}
for (var i=0; i<src.length; i++) pup.appendChild(src[i].cloneNode(true));
var aa = pup.getElementsByTagName("a");
for (var i=0; i<aa.length; i++) {if (aa[i].name) aa[i].name=""; if (aa[i].onmouseover) aa[i].onmouseover="";}
...
pup.style.top = top + "px";
                    ...
                    <body onclick="p()">
                        ...
                        <%= From production In Productions Select
                            <div class="p">
                                <h2><a name=<%= Uri.EscapeDataString(CaseEscapedNames(production.Key)) %>><%= MakeNonterminal(production.Key) %></a> ::=</h2>
                                <ul>
                        ...
                        <%= From terminal In Terminals Order By terminal Select
                            <div class="p">
                                <h2><a name=<%= Uri.EscapeDataString(CaseEscapedNames(normt(terminal))) %>><%= MakeTerminal(terminal) %></a></h2>
                                <ul>
    ...
    Shared Function MakeTerminal(ByVal t As String) As IEnumerable(Of XNode)
    ...
    Shared Function MakeNonterminal(ByVal p As String) As IEnumerable(Of XNode)
        Return <xml><a class="n"><%= p %></a></xml>.Nodes
    End Function

    Shared Function MakeEbnf(ByVal e As EBNF) As IEnumerable(Of XNode)
        Select Case e.Kind
    ...
    Shared Function flatten(ByVal ebnfs As IEnumerable(Of EBNF)) As IEnumerable(Of EBNF)
    ...
    Function MayBeEmpty(ByVal e As EBNF) As Boolean
        Select Case e.Kind
    ...
    Function MayStartWith(ByVal e As EBNF) As HashSet(Of String)
        Select Case e.Kind
    ...
    Sub UpdateMayBeFollowedBySet(ByVal e As EBNF, ByVal endings0 As HashSet(Of String), ByRef anyChanges As Boolean)
...
Class Antlr
    ...
    Public Shared Function Parse(ByVal src As String) As LinkedList(Of Production)
        ...
        While tokens.Count > 0
            ...
            If t = "grammar" Then
                ...
                If tokens.Count > 0 AndAlso tokens.First.Value = vbCrLf Then tokens.RemoveFirst()
            ElseIf t.StartsWith("//") Then
                productions.AddLast(New Production With {.Comment = t.Substring(2)})
                If tokens.Count > 0 AndAlso tokens.First.Value = vbCrLf Then tokens.RemoveFirst()
            ElseIf t = vbCrLf Then
                productions.AddLast(New Production)
            ElseIf tokens.Count > 0 AndAlso tokens.First.Value = ":" Then
                ...
                productions.AddLast(New Production With {.Comment = comment, .EBNF = p, .ProductionName = t, .RuleStartsOnNewLine = newline})
                While tokens.Count > 0 AndAlso tokens.First.Value.StartsWith("//")
                    productions.Last.Value.Comment &= tokens.First.Value.Substring(2) : tokens.RemoveFirst()
                    If tokens.First.Value = vbCrLf Then tokens.RemoveFirst()
                End While
            Else
                Throw New Exception("Unrecognized " & t)
    ...
    Private Shared Function ParseUnary(ByVal tokens As LinkedList(Of String), ByRef ExtraComments As String) As EBNF
