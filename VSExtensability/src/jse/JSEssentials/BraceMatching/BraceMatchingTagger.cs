...
namespace JSEssentials.BraceMatching
{
	internal class BraceMatchingTagger : ITagger<TextMarkerTag>
	{
		...
		//The event handlers update the current caret position of the CurrentChar property and raise the TagsChanged event.
		...
		//Implement the GetTags method to match braces either when the current character is an open brace or when 
		//the previous character is a close brace, as in Visual Studio. 
		...
		{
			if (spans.Count == 0 || SourceBuffer.CurrentSnapshot.Length == 0)   //there is no content in the buffer
				yield break;
			...
			//get the current char (or 0 if cursor is at the end of buffer) and the previous char
			char currentText = currentChar.Position >= currentChar.Snapshot.Length ? (char)0 : currentChar.GetChar();
