...
{             
    ...
    {            
        ...
        /// this parameter has a TypeNode for the type used as specialization of the generic type's first template param. 
        /// </param>
        private void AddExtensionMethod(XmlWriter writer, TypeNode type, Method extensionMethodTemplate, TypeNode specialization)
        {
            // Fixed the issue: http://sandcastle.codeplex.com/workitem/8852
            if (type != null)
            {
        ...
        private bool IsValidTemplateArgument(TypeNode type, TypeNode parameter)
        {
            ...
            if (itp == null) 
                throw new ArgumentException(
                    "The 'parameter' argument is null or not an 'ITypeParameter'.");
            ...
            bool constructor = ((itp.TypeParameterFlags & 
                TypeParameterFlags.DefaultConstructorConstraint) > 0);
        ...
        private bool DoParameterTypesMatch(ParameterList extensionParams, 
            ParameterList methodParams)
        {
            for (int i = 0; i < methodParams.Count; i++)
            {
                if (methodParams[i].Type.FullName != 
                    extensionParams[i + 1].Type.FullName)
                    return false;
            }
            return true;
        }  
